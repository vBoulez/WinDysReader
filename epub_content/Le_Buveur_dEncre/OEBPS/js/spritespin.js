...
(function($) {
  ...
  function load(opts){
    ...
    for (i = 0; i < src.length; i += 1 ) {
      img = new Image(); //hack PubCoder
    /*    var localImages = document.getElementsByTagName('img');
        //img = $(".spritespin-stage img[src$='" + src[i] + "']")[0];
        for (var i = 0; i < localImages.length; ++i) {
            if (localImages[i].getAttribute('src') == src[i]) {
               // img = localImages[i];
                 img.onload = img.onabort = img.onerror = tick; //hack PubCoder
                img.src = src[i];
                 images.push(img);
            }
        }
